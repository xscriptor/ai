...
function dstPath(target) {
  ...
  if (target === "anthropic") return join(home, ".claude", "skills", "xscriptor");
  const xdg = process.env.XDG_CONFIG_HOME || join(home, ".config");
  return join(xdg, "opencode", "skills", "xscriptor");
}

function srcPath() {
  const p = join(PKG_DIR, "skills", "web", "literature", "xscriptor");
  const r = join(REPO_DIR, "skills", "web", "literature", "xscriptor");
